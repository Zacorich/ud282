...
public class Game {
    ...
    public String checkGameWinner(char[][] grid) {
        ...
        int oCountToWinInLeftToRightHorizontal = 0;
        int xCountToWinInLeftToRightHorizontal = 0;

        int oCountToWinInRightToLeftHorizontal = 0;
        int xCountToWinInRightToLeftHorizontal = 0;

        //iterate the array
        for (int i = 0; i < grid.length; i++) {
            //columns and rows counters needs to be reset before inner loop
            int oCountToWinInColumns = 0;
            int xCountToWinInColumns = 0;
            int oCountToWinInRows = 0;
            int xCountToWinInRows = 0;
            for (int j = 0; j < grid[i].length; j++) {
                ...
                //check columns
                if (grid[i][j] == 'o') {
                    oCountToWinInColumns++;
                } else if (grid[i][j] == 'x') {
                    xCountToWinInColumns++;
                }

                //check rows of the same array by inverting indexes
                if (grid[j][i] == 'o') {
                    oCountToWinInRows++;
                } else if (grid[j][i] == 'x') {
                    xCountToWinInRows++;
                ...
                //check horizontal where indexes equal(left to right horizontal)
                if (i == j && grid[i][j] == 'o') {
                    oCountToWinInLeftToRightHorizontal++;
                }
                if (i == j && grid[i][j] == 'x') {
                    xCountToWinInLeftToRightHorizontal++;
                }

                //check right to left horizontal
                if (((i == 2 && j == 0) || (i == 1 && j == 1) || (i == 0 && j == 2)) && grid[i][j] == 'o') {
                    oCountToWinInRightToLeftHorizontal++;
                }

                if (((i == 2 && j == 0) || (i == 1 && j == 1) || (i == 0 && j == 2)) && grid[i][j] == 'x') {
                    xCountToWinInRightToLeftHorizontal++;
                }
                //check if we got any winner
                if (oCountToWinInColumns == 3 || oCountToWinInRows == 3 ||
                        oCountToWinInLeftToRightHorizontal == 3 || oCountToWinInRightToLeftHorizontal == 3) {
                    result = "O wins";
                } else if (xCountToWinInColumns == 3 || xCountToWinInRows == 3 ||
                        xCountToWinInLeftToRightHorizontal == 3 || xCountToWinInRightToLeftHorizontal == 3) {
                    result = "X wins";
                }
            }
        }

        //if no more move available on the board for next turn and no one won return "Tie"
        if(countFreeSpots == 0 && result.equals("None")) {
            result = "Tie";
        }

        //if we got to this point it means game is still going so return "None"
        return result;
    }
